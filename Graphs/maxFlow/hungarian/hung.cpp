...
int anterior[N]; //array for memorizing alternating paths
...
{
	S[x] = true; //add x to S
	anterior[x] = prevx; //we need this when augmenting
...
void augment() //main function of the algorithm
{
	...
	memset(anterior, -1, sizeof(anterior)); //init set anterior - for the alternating tree
	...
	if (y < n) //we found augmenting path!
	{
		...
		for (int cx = x, cy = y, ty; cx != -2; cx = anterior[cx], cy = ty)
			{
			ty = xy[cx];
			yx[cy] = cx;
			xy[cx] = cy;
...
int main(){
	...
	n = 3;
	vector<int> vv = {1,0,4,6,8,0,0,6,1};
	fr(i,9){
		cost[i/3][i%3] = vv[i];
	}
